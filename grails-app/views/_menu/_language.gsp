...
<%--<ul class="nav secondary-nav language-dropdown pull-right">--%>
	<li class="dropdown dropdown-btn js-language-dropdown">
		...
		<ul class="dropdown-menu dropdown-menu-dark" role="menu">
			...
			<!-- ${grailsApplication.config.grails.i18n.locales}get list of all locales available due to an existing property-file in /grails-app/i18n (set once in Bootstrap.groovy) -->
			<g:set var="allLocales" value="['en','de','es','us']"/>
                        
			<g:each status="i" var="locale" in="${allLocales}">
                            <% if ("${lang}"!="${locale}"){%>
                            <li>
                                <a class="js-language-link" title="${message(code: 'language.'+locale, default: locale)}" data-lang-code="${locale}" href="${currentURL+'?lang='+locale}">
